...
export default function SetupDoctorCenter() {
  ...
  async function diagnose() {
    ...
    if (!workerConfigured) {
      next.push({ id: 'worker', label: 'Background Worker', level: 'WARN', detail: '未設定です。Chat-only運用は可能です。Background/Guardianを使う場合は⚡からWorker URLと接続トークンを設定してください。' });
    } else {
      try {
        const health = await withTimeout(checkWorkerHealth(connection), 8000);
        workerHealthy = Boolean(health.ok);
        next.push({ id: 'worker', label: 'Background Worker', level: health.ok ? 'PASS' : 'FAIL', detail: health.ok ? 'Cloudflare Workerへ接続できました。' : 'Workerは応答しましたがhealthが正常ではありません。' });
      } catch (error) {
        next.push({ id: 'worker', label: 'Background Worker', level: 'FAIL', detail: `接続できません: ${error instanceof Error ? error.message : 'unknown error'}` });
      }
    }
    ...
    if (workerHealthy) {
      try {
        ...
        next.push({
          id: 'github-agent',
          label: 'GitHub Developer / Guardian',
          level: config.configured ? 'PASS' : 'WARN',
          detail: config.configured ? `GitHub実行設定済み・許可repo ${config.repositories.length}件。` : 'Workerは動いていますがGITHUB_TOKENまたはGITHUB_ALLOWED_REPOSが未設定です。',
          action: 'OPEN_DEVELOPER',
          actionLabel: 'GitHub Agentを確認',
        });
      } catch (error) {
        next.push({ id: 'github-agent', label: 'GitHub Developer / Guardian', level: 'WARN', detail: `GitHub Agent設定を確認できません: ${error instanceof Error ? error.message : 'unknown error'}`, action: 'OPEN_DEVELOPER', actionLabel: 'GitHub Agentを確認' });
      }
    } else {
      next.push({ id: 'github-agent', label: 'GitHub Developer / Guardian', level: 'WARN', detail: 'Background Workerが接続できるとGitHub Agent設定を診断できます。', action: 'OPEN_DEVELOPER', actionLabel: 'GitHub Agent画面を開く' });
    }
  ...
  return <>
    ...
    {open && <div className="doctor-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}><section className="doctor-sheet">
      ...
      <p className="doctor-footnote">WorkerやGitHubの秘密値そのものは表示しません。警告が残っていてもChat-only運用は可能な項目があります。</p>
    </section></div>}
  </>;
}
